...
1. КАРЬЕРНЫЙ РОСТ — роли в проектах должны отражать логичное развитие карьеры:
   - Ранние проекты (самые последние в списке) = junior/middle роли, небольшие команды
   - Средние проекты = middle/senior, растущая ответственность
   - Последние (первые в списке, самые свежие) = senior/lead/architect/CTO
   - Пример прогрессии: Junior Dev → Developer → Senior Developer → Tech Lead → CTO
   - НЕ делай все проекты с одинаковой ролью
...
2. СУММА ДЛИТЕЛЬНОСТЕЙ = ОБЩИЙ ОПЫТ:
   ...
   - Допускается ±6 месяцев (перекрытия, небольшие паузы)
   - Каждый проект: минимум 6 месяцев, максимум 60 месяцев (реалистично)

3. ТЕХНОЛОГИИ РАЗВИВАЮТСЯ:
   - В старых проектах — более старые/базовые стеки
   - В новых проектах — современные технологии
   - Не вставляй Kubernetes в проект 2010 года

4. РАЗМЕР КОМАНДЫ РАСТЁТ С КАРЬЕРОЙ:
   - Junior-проект: "3-5 человек", "небольшая команда"
   - Senior/Lead: "15-20 Backend, 8 Frontend, 3 QA"
   - CTO/Architect: "50+ Backend, 20 Frontend, 10 DevOps"

5. РЕАЛИЗМ ОПИСАНИЙ:
   - Описание компании — конкретное, с примерной нишей и масштабом
   - "Что реализовывал" — конкретные технические задачи, не абстрактные
   - Плохо: "Разрабатывал backend"
   - Хорошо: "Спроектировал и реализовал микросервисную архитектуру для обработки 50k RPS"
   - 5-8 конкретных пунктов в implementation

6. КОЛИЧЕСТВО ПРОЕКТОВ:
   - До 3 лет опыта: 2-3 проекта
   - 3-8 лет: 3-4 проекта
   - 8-15 лет: 4-5 проектов
   - 15+ лет: 5-6 проектов
  ...
  "projects": [
    {
      ...
      "duration": "XX месяцев",
      "description": "Чем занимается компания, масштаб, аудитория",
